...
This driver is simply defined as
<<driver>>=
RweaveRtf <- function(){
...
are only a handful in the original \code{RweaveLatexOptions} function. One new one is 
\code{resolution}.
...
Here is the syntax object for ``plain'' RTF that one writes
...
commented out the ones that are not used and added my new ones.
...
with \code{\textbackslash pngblip},
...
if (options$include) {
  if (options$png) {
    ...
    if(options$hex) {
      ...
      ## New approach to avoid cat which is slow writing to a share on my Windows machine

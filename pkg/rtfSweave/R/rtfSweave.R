SweaveSyntaxRtf <- 
  list(doc = "^@",
       ...
       docopt = "\\{\\\\SweaveOpts ([^\\}]*)\\}",
       ## escape the escape which escapes the curly
       ## escape the escape which escapes the escape
       ## a space
       ## - parens is for back reference 
       ## square brackets is to exclude backslash and right curly
       ## escape the escape of the end curly
       docexpr = "\\{\\\\Sexpr ([^\\}]*)\\}", # ... any valid S code, only curly brackets are not allowed
...
RweaveRtfOptions <- function (options) {
    ...
    NUMOPTS <- c("width", "height", "res", "pointsize")

...
Burke\cite{RTFPocketGuide}.  It covers important aspects of RTF and only after reading it did I
...
%% RTF\cite{RTFPocketGuide} and the latest RTF version specification can
...
type. Following the {\emph Pocket Guide}\cite{RTFPocketGuide},
...
Formatting instructions for the paragraph can be put on the first line
and following the \texttt{\textbackslash pard} command. To illustrate,
the following paragraph has six points (120 so-called twips\footnote{A twip is a standard unit of measurement
...
format is to indent everything after the first line with a space.
...
(\texttt{\textbackslash fs32}) and in bold (\texttt{\textbackslash
  b}). The command \texttt{\textbackslash keepn} keeps the heading
...
solution is to ``preprocess'' the file using something
...
@ 
With these kind of substitutions, the document can be
made more readable and much easier to write.

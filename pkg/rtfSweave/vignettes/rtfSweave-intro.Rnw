...
RTF and show how to use the \pkg{rtfSweave} package with \code{Sweave}
to make RTF statistical reports.  For me, the key to understanding RTF
was reading the excellent \emph{RTF Pocket Guide} by Sean
Burke\cite{guide}.  It covers important aspects of RTF and only after
reading it did I find that the RTF specifications published by
Microsoft made some sense. The \emph{Pocket Guide} can be purchased
and downloaded as a PDF from the publisher's website (currently
...
group containing metadata (optional). The font table is important
for \pkg{rtfSweave} because by default code chunks use 
\texttt{\textbackslash f2} and so this would typically be a monospaced
font. The markup for code chunks can be controlled with
the \texttt{rtf.Schunk} option. 
...
paragraphs are written using the following form.
...
is optional (and will not show up in the document) but helps readability.
...
commaand \texttt{\textbackslash outlinelevel1} is one of the few structural
...
substitutions and write an \textsf{R} function to
``preprocess'' the using text substitutions.
...
and after the paragraph, and text set in font zero at 12 points. Here is a 
simple example.
...
There is a ``short cut'' form to a paragraph which is just text ending
...
The good news is that \pkg{rtfSweave} handles this. Like in
...
command. This makes for a smaller document but special steps
need to be taken to embed the figures in the document.

...
Word user and want fine control over the format of the document,
there may be an advantage to writing the RTF directly
rather than using a system that involves converting one output format
to a  document conversion program.
